---
id: "wjl1224734792/jarvis-agent-factory"
name: "Wjl1224734792/Jarvis-Agent-Factory"
url: "https://github.com/Wjl1224734792/Jarvis-Agent-Factory"
date: "2026-05-21"
source: "awesome-mcp"
...
replacement_target: ""
related_articles: []
pros: ["Recently updated (2026-05-21)","MIT license","6 GitHub stars","GitHub Actions/CI detected"]
cons: ["Integration may take more than a quick install"]
readme_quality: 85
has_ci: true
has_tests: true
setup_steps_count: 3
dependency_files: [{"name":"package.json","summary":"deps @hono/node-server, @modelcontextprotocol/sdk, hono, yaml, @eslint/js, @types/node, @typescript-eslint/eslint-plugin, @typescript-eslint/parser; scripts clean, prebuild, lint, lint:fix, test, test:watch, typecheck, build"}]
install_commands: ["npm i -g jarvis-agent-factory   # 安装 CLI（零原生依赖，node:sqlite 内置）","Claude Code","npm test","git tag -a v<version> -m \"v<version> - <概要>\"","git push origin main && git push origin v<version>","npm view jarvis-agent-factory version"]
risk_flags: []
status: "new"
---
...
## Why it matters
Wjl1224734792/Jarvis-Agent-Factory matches your AI development source filters and scored strongly enough to review before it disappears into the daily noise.
...
Dependency files:
- package.json: deps @hono/node-server, @modelcontextprotocol/sdk, hono, yaml, @eslint/js, @types/node, @typescript-eslint/eslint-plugin, @typescript-eslint/parser; scripts clean, prebuild, lint, lint:fix, test, test:watch, typecheck, build
...
- npm view jarvis-agent-factory version
...
> **v4.6.0** — 新增 /repowiki 项目知识库指令（add/ingest/query/list/read/delete/lint），持久化 Markdown Wiki
...
| 项目介绍 | `/commands-reference.html` | 项目概览 · 32条指令流程详解 · 71个Agent目录 · 流水线架构 · 使用推荐 |
| 操作指南 | `/guide.html` | 完整使用指南：架构原理 · 编排机制 · 15条流水线 · 32条指令 · 71个Agent · 场景推荐 |
...
📖 **引擎 MCP 配置、Visual Primitives MCP 接入、开发环境配置** → [QUICKSTART.md](./QUICKSTART.md)
...
每个 Claude Code 命令的完整介绍，包含项目概览、32条指令的详细流程（含 Gate 序列和 Agent spawn 关系）、71个智能体目录、流水线架构和使用推荐：[项目介绍页](https://github.com/Wjl1224734792/Jarvis-Agent-Factory/blob/main/web/public/commands-reference.html)
