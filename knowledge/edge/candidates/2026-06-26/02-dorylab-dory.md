...
replacement_target: ""
related_articles: []
pros: ["Recently updated (2026-06-26)","Apache-2.0 license","237 GitHub stars","GitHub Actions/CI detected"]
cons: ["Integration may take more than a quick install","README mentions credentials or API tokens"]
readme_quality: 100
has_ci: true
has_tests: false
setup_steps_count: 4
dependency_files: [{"name":"package.json","summary":"deps @playwright/test; scripts dev, prebuild, build, build:bootstrap, deploy, start, studio, pglite:studio"}]
install_commands: ["docker run -d --name dory \\","docker compose up -d","codex mcp add dory --url http://127.0.0.1:3318/api/mcp","codex mcp list","claude mcp add --transport http dory http://127.0.0.1:3318/api/mcp","claude mcp list"]
...
## Why it matters
dorylab/dory matches your AI development source filters and scored strongly enough to review before it disappears into the daily noise.
...
Dependency files:
- package.json: deps @playwright/test; scripts dev, prebuild, build, build:bootstrap, deploy, start, studio, pglite:studio
...
Dory combines a familiar SQL workspace with AI assistance and MCP support.
You can write SQL manually, ask AI to help with queries, or let external agents like Claude and Codex explore your database through Dory MCP.
...
<!-- > Explore data with You and AI, together. -->
